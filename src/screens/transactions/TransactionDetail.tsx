          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent>
                ...
                <div className="space-y-4">
                  {transaction.milestones.map((milestone, index) => (
                    <div key={milestone.id} className="flex items-center justify-between p-4 rounded-lg border">
                      <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm capitalize">{milestone.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card>
              ...
              <CardContent className="flex flex-wrap gap-3">
                {transaction.status === "pending" && isBuyer && <Button>Fund Transaction</Button>}
                {transaction.status === "funded" && !isBuyer && <Button>Start Work</Button>}
                {transaction.status === "in_progress" && !isBuyer && <Button>Mark as Delivered</Button>}
                  ...
                  {transaction.fundedAt && (
                    <div className="flex gap-4">
                      ...
                      <div className="pb-4">
                        ...
                        <p className="text-sm text-muted-foreground">{formatDateTime(transaction.fundedAt)}</p>
                      </div>
                    </div>
                  )}
                  {transaction.releasedAt && (
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{formatDateTime(transaction.releasedAt)}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <p className="text-sm text-muted-foreground mb-2">{isBuyer ? "You (Buyer)" : "Buyer"}</p>
                  <div className="flex items-center gap-3">
                ...
                <Separator />
                <div>
                  <p className="text-sm text-muted-foreground mb-2">{!isBuyer ? "You (Seller)" : "Seller"}</p>
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                      <User className="h-5 w-5 text-primary" />
                    </div>
